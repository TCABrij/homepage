...
function getRandomId() {
  const letters =
    "ABCDEFGHIJKLMNOPQRSTUVQXYZabcdefghijklmnopqrstuvwxyz1234567890";
  ...
  while (id.length <= idLength) {
    ...
    if (id.length === idLength) return id;
  }
}
...
function updateById(linkObj) {
  ...
  const index = parsedLink.links.findIndex(
    (link, index) => link.id == linkObj.id
  );
...
function sortLinkCards() {
  let links = localStorage.getItem("links");
  let linksObj = JSON.parse(links);

  // sort Alphabetically
  linksObj.links.sort((a, b) => {
  ...
  let sortedLinks = JSON.stringify(linksObj)
  localStorage.setItem('links', sortedLinks)
